...
public class UpdateProductController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(isMulti){
            ...
            boolean isAvailable=Boolean.valueOf((String)params.get("status"));
            ...
            if(!itemName.split("\\.")[1].equals("jpg")&&!itemName.split("\\.")[1].equals("png")){
                check=false;
                error.setImageError("Must select file .jpg or .png !!!");
            }
            if(check){                               
                    url=SUCCESS;
                    Date date=new Date();
                    ...
                    try{
                        int num=dao.updateProduct(proId, name, description, price, quantity, categoryId, isAvailable, updateDate);
                        if(num==1){
                            dao.updateImage(itemName, proId);
                            request.setAttribute("SUCCESS_UPDATE", "Update successful");
                        }                    
